...
func (serv *RPCServer) NewInput(a *rpctype.NewInputArgs, r *int) error {
	...
	log.Logf(0, "new input from %v for syscall %v (signal=%v, cover=%v, memCover=%v, duCover=%v)",
	...
	serv.mgr.writeTestLog(fmt.Sprintf("[%v] %v %v %v(%v) %v\n", currTime, hash.String(a.RPCInput.Prog), len(a.Cover), len(a.MemCover), memDiff, a.Metric))
...
func (serv *RPCServer) Poll(a *rpctype.PollArgs, r *rpctype.PollRes) error {
	...
	log.Logf(0, "poll from %v: candidates=%v inputs=%v maxsignal=%v needCand=%v f.inputs=%v mgr.cand=%v workq=%v",
		a.Name, len(r.Candidates), len(r.NewInputs), len(r.MaxSignal.Elems), a.NeedCandidates, len(f.inputs), serv.mgr.getCandidateLen(), a.WorkQueue)
	return nil
}
