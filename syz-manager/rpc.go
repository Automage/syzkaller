...
func (serv *RPCServer) Poll(a *rpctype.PollArgs, r *rpctype.PollRes) error {
	...
	log.Logf(0, "poll from %v: candidates=%v inputs=%v maxsignal=%v needCand=%v f.inputs=%v mgr.cand=%v workq=%v, procs=%v",
		a.Name, len(r.Candidates), len(r.NewInputs), len(r.MaxSignal.Elems), a.NeedCandidates, len(f.inputs), serv.mgr.getCandidateLen(), a.WorkQueue, a.Procs)
	return nil
}
